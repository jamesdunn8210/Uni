...
public class TileRenderer {
	...
	private HashMap<String, Texture> tile_textures; //texture name is key
	private Model model;
	...
	public TileRenderer() {
		...
		for(int i = 0; i< Tile.tiles.length; i++) {
			if(Tile.tiles[i] !=null) {
				if(!tile_textures.containsKey ( Tile.tiles[i].getTexture())) { //name of texture 
	...
	public void renderTile(byte id, int x, int y, Shader shader, Matrix4f world, Camera cam) {
		...
		Matrix4f tile_pos = new Matrix4f().translate(new Vector3f(x, y, 0)); //*2
